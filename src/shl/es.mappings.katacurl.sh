...
function es-copy-mapping-curl {
  ...
  # CURL METHODE
  # for-each [ index ]
  #   es-export ((_settings,_mappings), _aliases, _warmers) $index ${ES_URL_FROM}_url
  #   es-import ((_settings,_mappings), _aliases, _warmers) $index ${ES_URL_TO}_url
  # !!! _settings,_mappings couple qui ne peut pas etre separes pour une seule passe
  ...
  cmd="curl -XPUT '${ES_URL_TO}/${ARG1}' -d @/tmp/${ARG1}.smaw.json && echo"; 
  echo "RUN ${cmd}"; 
  eval ${cmd};
}
...
function es-process-data {
  ...
  es-copy-mapping-curl ${ARG1};   #copy MAPPING from ${ES_URL_FROM} to ${ES_URL_TO}
}
